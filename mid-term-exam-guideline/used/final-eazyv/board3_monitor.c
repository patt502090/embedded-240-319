...
static void timer0_init_1ms(void){
  ...
  TCCR0A=(1<<WGM01); TCCR0B=(1<<CS01)|(1<<CS00); OCR0A=249; TIMSK0=(1<<OCIE0A);
}
...
static void timer2_init_1kHz(void){
  ...
  TCCR2A=(1<<WGM21); TCCR2B=(1<<CS22); OCR2A=249; TIMSK2=(1<<OCIE2A);
}
ISR(TIMER2_COMPA_vect){
  ...
  if (code & 0x40) SEG_PORT_G |= (1<<SEG_BIT_G);
...
static void compose_run(uint16_t pills, uint8_t prog){
  ...
  seg_frame[0]=dt[L/10]; seg_frame[1]=dt[L%10];
  seg_frame[2]=dt[R/10]; seg_frame[3]=dt[R%10];
}
static void compose_clean(void){
  seg_frame[0]=encC('C'); seg_frame[1]=encC('L');
  seg_frame[2]=encC('E'); seg_frame[3]=encC('n');
}
...
/* RX parse */
#define RXN 32
static char rxbuf[RXN]; static uint8_t rxi=0;
static void poll_serial(void){
  while (UCSR0A&(1<<RXC0)){
    char c = UDR0; last_rx_ms=g_ms;
    if (c=='\r') continue;
    if (c=='\n'){
      rxbuf[rxi]='\0';
      if (!strncmp(rxbuf,"PILLS:",6)){
        int v=atoi(rxbuf+6); if (v>=0&&v<=999) pills=(uint16_t)v;
      } else if (!strncmp(rxbuf,"PROGRESS:",9)){
        int v=atoi(rxbuf+9); if (v>=0&&v<=100) progress=(uint8_t)v;
      ...
      if (rxi<RXN-1) rxbuf[rxi++]=c; else rxi=0;
    }
  }
}
...
int main(void){
  // IO directions
  SEG_DDR_C |= SEG_MASK_CF;  SEG_DDR_G |= (1<<SEG_BIT_G);
  DIGIT_DDR |= DIGIT_MASK;
  LED_DDR   |= (1<<LED_G)|(1<<LED_Y)|(1<<LED_R)|(1<<LED_B);
  PACK_DDR  |= (1<<PACK_PIN);
  SOL_DDR   |= (1<<SOL_PIN);
  // init outputs
  SEG_PORT_C &= ~SEG_MASK_CF; SEG_PORT_G &= ~(1<<SEG_BIT_G); DIGIT_PORT &= ~DIGIT_MASK;
  LED_PORT &= ~((1<<LED_G)|(1<<LED_Y)|(1<<LED_R)|(1<<LED_B));
  PACK_PORT &= ~(1<<PACK_PIN); SOL_PORT &= ~(1<<SOL_PIN);
  ...
  last_rx_ms=g_ms; mode=M_RUN; compose_run(pills,progress);
  ...
  for(;;){
    ...
    if (mode==M_TIMEOUT){
      compose_timeout(); leds(false,false,true);       // แดงค้าง
      if ((g_ms - last_rx_ms) <= 30000UL) mode=M_RUN; // มีข้อมูลใหม่ → กลับ RUN
    }
    else if (mode==M_CLEAN){
      unsigned long dt=g_ms - clean_t0;
      // 0..3s เปิดโซลินอยด์ / 0..5s เปิด packaging
      if (dt<3000UL) SOL_PORT |= (1<<SOL_PIN); else SOL_PORT &= ~(1<<SOL_PIN);
      if (dt<5000UL) PACK_PORT |= (1<<PACK_PIN); else PACK_PORT &= ~(1<<PACK_PIN);
      compose_clean(); leds(false,false,false);
      if (dt>=5000UL){
        mode=M_RUN; pills=0; progress=0;
        PACK_PORT&=~(1<<PACK_PIN); SOL_PORT&=~(1<<SOL_PIN);
      ...
      if (progress>=100){
        mode=M_CLEAN; clean_t0=g_ms; usart0_send_string("RESET\n");
      }
    }
